CREATE OR REPLACE FUNCTION public.validate_attendance_report(p_attendance_report_id uuid, p_action text, p_rejection_reason text DEFAULT NULL::text, p_professional_amount numeric DEFAULT 0, p_foundation_amount numeric DEFAULT 0, p_total_amount numeric DEFAULT 0, p_payment_method text DEFAULT 'dinheiro'::text)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF NOT user_has_role(ARRAY['director'::employee_role, 'coordinator_madre'::employee_role, 'coordinator_floresta'::employee_role, 'coordinator_atendimento_floresta'::employee_role]) THEN
  ...
  IF p_action = 'validate' THEN
    ...
    -- USAR DATA REAL DO ATENDIMENTO
    attendance_date := attendance_record.start_time::date;
    ...
    final_payment_method := COALESCE(NULLIF(p_payment_method, ''), 'dinheiro');
    ...
    -- Processar materiais utilizados (PERMITE ESTOQUE NEGATIVO)
    IF attendance_record.materials_used IS NOT NULL THEN
      ...
      LOOP
        ...
        -- REMOVIDA VERIFICAÇÃO DE ESTOQUE SUFICIENTE - permite estoque negativo
        IF current_stock_qty IS NOT NULL THEN
          DECLARE
          ...
          BEGIN
            -- Criar movimentação de estoque COM DATA DO ATENDIMENTO
            -- CORRIGIDO: usar 'out' ao invés de 'saida' para compatibilidade com constraint
            ...
            -- Atualizar estoque (permite valor negativo)
            UPDATE stock_items 
            SET 
    ...
    -- 1. RECEITA DA SESSÃO COM DATA DO ATENDIMENTO
    IF final_total_amount > 0 THEN
      INSERT INTO financial_records (
        ...
        attendance_record.client_id,
        auth.uid(),
        'Atendimento validado - Método: ' || final_payment_method || ' - Valor total: R$ ' || final_total_amount
      );
    END IF;
    
    -- 2. PAGAMENTO PROFISSIONAL COM DATA DO ATENDIMENTO
    ...
    -- 3. RECEITA DA FUNDAÇÃO COM DATA DO ATENDIMENTO
    ...
    -- 4. CUSTO DE MATERIAIS COM DATA DO ATENDIMENTO
  ...
  ELSIF p_action = 'reject' THEN
    ...
    -- Voltar status do schedule para scheduled
    UPDATE schedules 
