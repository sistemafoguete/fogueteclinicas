-- Corrigir função create_employee_direct para incluir provider_id
...
BEGIN
  ...
  -- Create identity record se não existe - CORRIGIDO com provider_id
  IF NOT EXISTS (SELECT 1 FROM auth.identities WHERE user_id = new_user_id AND provider = 'email') THEN
    INSERT INTO auth.identities (
      ...
      gen_random_uuid(),
      new_user_id,
      new_user_id, -- provider_id deve ser o mesmo que user_id para provider email
      jsonb_build_object(
