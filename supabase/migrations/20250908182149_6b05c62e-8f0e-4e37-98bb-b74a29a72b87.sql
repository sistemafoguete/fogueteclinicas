-- Create sample employees with matching login/password
-- First, let's create some employee profiles

-- Insert sample employee profiles with different roles
...
-- Nurses
('44444444-4444-4444-4444-444444444444', 'Enfermeira Paula Costa', 'nurse', '(11) 99999-4444', '444.444.444-44', 'PR-44.444.444', '2022-02-01', 'Enfermagem', 5000.00, true),
('55555555-5555-5555-5555-555555555555', 'Enfermeiro João Oliveira', 'nurse', '(11) 99999-5555', '555.555.555-55', 'RS-55.555.555', '2022-05-15', 'Enfermagem', 5000.00, true),

-- Therapists
('66666666-6666-6666-6666-666666666666', 'Fisioterapeuta Lucia Pereira', 'therapist', '(11) 99999-6666', '666.666.666-66', 'SC-66.666.666', '2021-08-30', 'Fisioterapia', 6000.00, true),
('77777777-7777-7777-7777-777777777777', 'Psicoterapeuta Roberto Alves', 'therapist', '(11) 99999-7777', '777.777.777-77', 'GO-77.777.777', '2022-01-10', 'Psicoterapia', 6000.00, true),

-- Administrative Staff
('88888888-8888-8888-8888-888888888888', 'Recepcionista Carla Souza', 'administrative', '(11) 99999-8888', '888.888.888-88', 'MT-88.888.888', '2022-07-01', 'Recepção', 3500.00, true),
('99999999-9999-9999-9999-999999999999', 'Assistente Marcos Ferreira', 'administrative', '(11) 99999-9999', '999.999.999-99', 'BA-99.999.999', '2022-09-15', 'Administrativo', 3500.00, true),

-- Staff
('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 'Auxiliar Sandra Rodrigues', 'staff', '(11) 99999-aaaa', '111.222.333-44', 'CE-11.222.333', '2022-11-01', 'Auxiliar Geral', 2800.00, true);
...
SELECT 
  ...
  CASE 
    WHEN p.employee_role = 'therapist' AND p.name LIKE '%Fisio%' THEN 'Fisioterapia'
    WHEN p.employee_role = 'therapist' AND p.name LIKE '%Psico%' THEN 'Psicoterapia'  
    WHEN p.employee_role = 'nurse' THEN 'Enfermagem'
    WHEN p.employee_role IN ('director', 'coordinator_madre', 'coordinator_floresta') THEN 'Gestão'
    ELSE 'Geral'
  END as specialization,
  CASE 
    WHEN p.employee_role = 'therapist' AND p.name LIKE '%Fisio%' THEN 'CREFITO-123456'
    WHEN p.employee_role = 'therapist' AND p.name LIKE '%Psico%' THEN 'CRP-654321'
    WHEN p.employee_role = 'nurse' THEN 'COREN-789012'
    ELSE NULL
