-- Verificar funções disponíveis do pgcrypto
SELECT proname FROM pg_proc WHERE proname LIKE '%crypt%' OR proname LIKE '%hash%';

-- Usar abordagem que funciona com Supabase Auth diretamente
...
BEGIN
  ...
  -- Usar hash simples compatível com Supabase (bcrypt format)
  -- Para desenvolvimento, vamos usar um hash MD5 temporário
  hashed_password := '$2a$10$' || encode(digest(p_password || new_user_id::text, 'sha256'), 'hex');
