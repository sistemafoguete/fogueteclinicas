-- Primeiro, vamos corrigir a função de validação para processar corretamente os materiais
-- e adicionar suporte para valores profissional e fundação
...
BEGIN
  ...
  IF p_action = 'validate' THEN
    ...
    -- 2. Valor para o profissional (se informado)
    IF p_professional_amount > 0 THEN
      INSERT INTO financial_records (
        type,
        category,
        description,
    ...
    -- 3. Valor para a fundação (se informado)
    IF p_foundation_amount > 0 THEN
      INSERT INTO financial_records (
        type,
        category,
        description,
