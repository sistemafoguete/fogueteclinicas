-- Adicionar campos de valor profissional e valor da instituição aos relatórios de atendimento
ALTER TABLE public.attendance_reports 
ADD COLUMN IF NOT EXISTS professional_amount numeric DEFAULT 0,
ADD COLUMN IF NOT EXISTS institution_amount numeric DEFAULT 0;

-- Adicionar comentários para documentar os campos
COMMENT ON COLUMN public.attendance_reports.professional_amount IS 'Valor destinado ao profissional';
COMMENT ON COLUMN public.attendance_reports.institution_amount IS 'Valor destinado à instituição';

-- Atualizar função de validação para processar os novos valores financeiros
...
BEGIN
  ...
  IF p_action = 'validate' THEN
    ...
    IF attendance_record.amount_charged > 0 THEN
      INSERT INTO financial_records (
        ...
        'consultation',
        attendance_record.attendance_type || ' - ' || attendance_record.patient_name,
        attendance_record.amount_charged,
        CURRENT_DATE,
        'cash',
        attendance_record.client_id,
        attendance_record.employee_id,
        auth.uid(),
        'Atendimento validado - Valor total da sessão'
      );
    END IF;
    ...
    IF final_professional_amount > 0 THEN
      INSERT INTO financial_records (
        ...
        'Pagamento profissional - ' || attendance_record.patient_name,
